<template>
    ...
    <table id="field-key-list-table" class="table">
        ...
        <tbody>
          ...
          :mobile-user="mobileUser" :highlighted="highlighted"
          @show-code="showPopover"
          @revoke="revokeModal.show({ mobileUser: $event })"/>
        </tbody>
    </table>
    <p v-if="mobileUsers.dataExists && mobileUsers.length === 0"
...
import FieldKeyNew from '../../field-key/new.vue';
...
export default {
    ...
    components:{DateTime, TimeAndUser, FieldKeyNew, DocLink, RowMobileUser},
    setup() {
  ...
  computed: {
    actionsId() {
      return `field-key-row-actions${this.mobileUser.id}`;
    }
  },
  methods:{
    afterCreate(mobileUser) {
      this.createModal.hide();
      this.highlighted = mobileUser.id;
      this.alert.success(this.$t('alert.create', mobileUser));

    },
    showPopover(mobileUser, link) {
      this.popover.target = link;
      this.popover.mobileUser = mobileUser;
    },
  }
}
</script>
